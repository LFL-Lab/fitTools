...
10/21/2021 - Rahel added this file to make c modifications to the python given

@author: lfl
"""
import pyximport
import cython as cnp
import numpy as np
...
cdef expectedShift(q0, f0, ls, phi, delta=170e-6):
...
cdef f_n_phi(phi, n, L=1.82897e-9, C=0.739929e-12, Ls=20.8475e-12,
...
cdef uint16_to_mV(data, bitshifted=False):
    '''
...
cdef getNumberSegmentsForBigData(fpath, segmentSizeGB=10):
...
cdef loadChunk(fpath, elementsPerSegment, segment, nChannels=2):
...
cdef loadAlazarData(fpath, nChannels=2):
...
cdef HannConvolution(data,avgTime,sampleRate):
...
cdef FlattopConvolution(data,avgTime,sampleRate):
...
cdef GaussianConvolution(data,avgTime,sampleRate):
...
cdef BoxcarConvolution(data,avgTime,sampleRate):
...
cdef BoxcarDownsample(data,avgTime,sampleRate,returnRate=False):
...
cdef plotComplexHist(I,Q,bins=(80,80),figsize=[9,6],returnHistData=False):
...
cdef make_ellipses(gmm,ax,colors):
...
cdef make_ellipses2(means,varis,ax,colors):
    '''Adds colored ellipses illustrating mean and std dev of mode to the given pyplot subplot.
...
cdef getGaussianSNR(gmm,mode1=0,mode2=1):
...
cdef getDataSNR(mode1data,mode2data):
...
cdef predictWithBayes(gmm,data,nMemory=1):
...
cdef extractTimeBetweenTrapEvents(nEst,time):
...
cdef extractLifetimes(nEst,time):
...
cdef extractAntiLifetimes(nEst,time):
...
cdef extractDurations(nEst,time):
...
cdef plotTimeSeries(data,nEst,time,start,stop,zeroTime=False):
    '''
...
cdef extractLifetimesWithModes(nEst,time):
...
cdef extractLifetimesWithTwoModes(nEst,time):
...
cdef fitExpDecay(dist,t,cut=0,returnTauDetector=True,returnSGDIST=False):
...
cdef fitAndPlotExpDecay(dist,cut=None,returnTaus=False,figsize=[3.325,3.325],bins=100):
...
cdef plotBurstSearch(nEst,avgTime,sampleRate,method='boxcar'):
...
cdef getBurstIndices(burstSearch,threshold,nConsecutive=5):
...
cdef getRollingRates(nEst,sampleRate,windowDuration):
    '''
...
cdef getGaussian(args,p,means=None,varis=None):
    '''
...
cdef fitGaussiansIndependent(quietSubs):
    '''
...
cdef getGaussianProbabilities(DATA,amps,means,varis):
    '''
...
cdef predictWithBayes2(probs,nMemory=1):
...
cdef _gaussian(args,p,means=None,varis=None):
    ...
    args : tuple (xx,yy)
     ...
     : 1-d array-like variable length. MUST BE CALLED WITH PRECEEDING ASTERISK
        asterisk is required so py*pthon treats this as multiple arguments rather than a single list.
        a list of parameters for each mode in gaussian distribution.
        for M modes in distribution, should have length M or M*6 per conditions below.
        IF means and varis are None, follows order: amplitude,x0,y0,sigmax,sigmay,angle.
        IF means and varis ARE PROVIDED, then p contains only amplitudes
...
cdef getQuietSubsets(DATA,nEst,time,thresholds):
    '''
...
cdef getQuietSubsets2(DATA,nEst,nSubs=100):
    '''
...
cdef plotQuietSubsets(quietSubs):
...
cdef getQuietThresholds(lifetimes):
    '''
...
cdef subtractionGaussianFit(DATA,nModes):
    '''
    Fit each mode independently to get the means and variances
...
cdef PoissonCorrection(ti,td,tinot,tdnot):
    vs = tdnot/tinot
    us = td/ti
